...
def validate_layouts_for_channel_selection(input_layout, output_layout):
    """
    ...
    if input_layout[-1] < output_layout[-1]:
        raise ValueError(
            "Input/superset final output dimension must be >= output final dimension. "
            f"Got input_layout[-1]={input_layout[-1]} < output_layout[-1]={output_layout[-1]}"
        )
...
def select_final_output_channels(weights, biases, out_dim: int):
    """
    Keeps the first `out_dim` output channels of the FINAL layer only.

    Input:
      weights: list/tuple of tensors, final weight [B, in_dim, full_out_dim, 1]
      biases:  list/tuple of tensors, final bias   [B, full_out_dim, 1]

    Output:
      same hidden layers, but final layer reduced to:
        final weight [B, in_dim, out_dim, 1]
        final bias   [B, out_dim, 1]
    """
    if out_dim <= 0:
        raise ValueError(f"out_dim must be positive, got {out_dim}")

    new_weights = list(weights[:-1])
    new_biases = list(biases[:-1])

    final_w = weights[-1]
    final_b = biases[-1]

    if final_w.shape[2] < out_dim:
        raise ValueError(
            f"Cannot select {out_dim} output channels from final weight with shape {final_w.shape}"
        )
    if final_b.shape[1] < out_dim:
        raise ValueError(
            f"Cannot select {out_dim} output channels from final bias with shape {final_b.shape}"
        )

    final_w = final_w[:, :, :out_dim, :]
    final_b = final_b[:, :out_dim, :]

    new_weights.append(final_w)
    new_biases.append(final_b)
...
class PairedNavierINRPredictionDataset(torch.utils.data.Dataset):
    ...
    Returns:
      ...
      - input INR params (used as base for residual editing)
      - ground-truth output INR params (original target topology, e.g. 1 output)
      - sample index
    """
    ...
    def __init__(
        ...
        output_layer_layout=None,
        image_size=(129, 129),
    ):
        super().__init__()

        if input_layer_layout is None or output_layer_layout is None:
            raise ValueError("Both input_layer_layout and output_layer_layout must be provided")

        validate_layouts_for_channel_selection(input_layer_layout, output_layer_layout)

        self.input_layer_layout = input_layer_layout
        self.output_layer_layout = output_layer_layout
...
def evaluate(
    ...
    for i, batch in enumerate(loader):
        ...
        weights = list(input_wb.weights)
        biases = list(input_wb.biases)

        delta_weights, delta_biases = model(params, weights, biases)
        pred_full_weights, pred_full_biases = residual_param_update(weights, biases, delta_weights, delta_biases)

        pred_out_weights, pred_out_biases = select_final_output_channels(
            pred_full_weights,
            pred_full_biases,
            out_dim=output_final_dim,
        )

        pred_out = inr_model(pred_out_weights, pred_out_biases)  # [B,P,C_out]
        gt_out = inr_model(gt_out_wb.weights, gt_out_wb.biases)  # [B,P,C_out]
...
def main(args=None):
    ...
    input_layer_layout = conf["data"]["input_layer_layout"]    # superset/input topology
    output_layer_layout = conf["data"]["output_layer_layout"]  # target topology used for inference/loss
    ...
    best_model_path = conf.get("best_model_path", "best_scalegmn_navier_editing_channel_select.pt")

    output_final_dim = output_layer_layout[-1]
    ...
    for epoch in epoch_iter:
        for i, batch in enumerate(train_loader):
            ...
            # Residual editing in input/superset topology
            delta_weights, delta_biases = net(params, weights, biases)
            pred_full_weights, pred_full_biases = residual_param_update(weights, biases, delta_weights, delta_biases)

            # After editing, reduce final layer to the target output dimensionality
            pred_out_weights, pred_out_biases = select_final_output_channels(
                pred_full_weights,
                pred_full_biases,
                out_dim=output_final_dim,
            ...
            # Build predicted 1-output INR and compare against original 1-output GT INR
            ...
            if global_step > 0 and (global_step % conf["train_args"]["eval_every"] == 0):
                val_dict = evaluate(
                    ...
                    device,
                    inr_model,
                    output_final_dim=output_final_dim,
                )
