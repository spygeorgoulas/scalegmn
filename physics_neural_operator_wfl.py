...
def compute_parameter_loss(pred_weights, pred_biases, gt_weights, gt_biases):
    """
    Mean of per-tensor MSE across all weights and biases.
    This prevents very large layers from dominating purely due to size.
    """
    losses = []

    if len(pred_weights) != len(gt_weights):
        raise ValueError(
            f"Mismatch in number of weight tensors: {len(pred_weights)} vs {len(gt_weights)}"
        )
    if len(pred_biases) != len(gt_biases):
        raise ValueError(
            f"Mismatch in number of bias tensors: {len(pred_biases)} vs {len(gt_biases)}"
        )

    for i, (pw, gw) in enumerate(zip(pred_weights, gt_weights)):
        if pw.shape != gw.shape:
            raise ValueError(
                f"Weight tensor shape mismatch at layer {i}: pred {pw.shape}, gt {gw.shape}"
            )
        losses.append(((pw - gw) ** 2).mean())

    for i, (pb, gb) in enumerate(zip(pred_biases, gt_biases)):
        if pb.shape != gb.shape:
            raise ValueError(
                f"Bias tensor shape mismatch at layer {i}: pred {pb.shape}, gt {gb.shape}"
            )
        losses.append(((pb - gb) ** 2).mean())
...
def evaluate(
    ...
    for i, batch in enumerate(loader):
        ...
        param_loss = compute_parameter_loss(
            pred_out_weights,
            pred_out_biases,
            gt_out_wb.weights,
            gt_out_wb.biases,
        )
        param_mse_losses.append(param_loss.detach().unsqueeze(0).cpu())
        ...
        field_mse = ((pred_out - gt_out) ** 2).mean(dim=(1, 2))  # [B]
        field_mse_losses.append(field_mse.detach().cpu())
    ...
    avg_field_mse = torch.cat(field_mse_losses).mean()
    avg_param_mse = torch.cat(param_mse_losses).mean()
    avg_rel_l2 = torch.cat(rel_l2_losses).mean()

    if not torch.isfinite(avg_field_mse):
        raise RuntimeError("avg_field_mse is non-finite during evaluation")
    if not torch.isfinite(avg_param_mse):
        raise RuntimeError("avg_param_mse is non-finite during evaluation")
    ...
    return {
        "avg_field_mse": avg_field_mse,
        "avg_param_mse": avg_param_mse,
...
def main(args=None):
    ...
    lambda_field = float(conf["train_args"].get("lambda_field", 1.0))
    lambda_param = float(conf["train_args"].get("lambda_param", 0.1))
    ...
    for epoch in epoch_iter:
        for i, batch in enumerate(train_loader):
            ...
            field_loss = field_criterion(pred_out, gt_out)
            loss = lambda_field * field_loss + lambda_param * param_loss

            if not torch.isfinite(field_loss):
                raise RuntimeError(f"Non-finite field_loss detected during training: {field_loss.item()}")
            if not torch.isfinite(param_loss):
                raise RuntimeError(f"Non-finite param_loss detected during training: {param_loss.item()}")
            if not torch.isfinite(loss):
                raise RuntimeError(f"Non-finite total loss detected during training: {loss.item()}")

            loss.backward()

            log = {
                "train/loss_total": float(loss.item()),
                "train/loss_field": float(field_loss.item()),
                "train/loss_param": float(param_loss.item()),
                "train/lambda_field": lambda_field,
                "train/lambda_param": lambda_param,
                "epoch": epoch,
            ...
            epoch_iter.set_description(
                ...
                f"train_total={loss.item():.4e} | "
                f"field={field_loss.item():.4e} | "
                f"param={param_loss.item():.4e} | "
                f"test_rel_l2={test_rel_l2:.4e}"
            )
            ...
            if global_step > 0 and (global_step % conf["train_args"]["eval_every"] == 0):
                ...
                val_field_mse = float(val_dict["avg_field_mse"])
                val_param_mse = float(val_dict["avg_param_mse"])
                val_rel_l2 = float(val_dict["avg_rel_l2"])

                test_field_mse = float(test_dict["avg_field_mse"])
                test_param_mse = float(test_dict["avg_param_mse"])
                ...
                if run is not None:
                    eval_log = {
                        "train/avg_field_mse": float(train_dict["avg_field_mse"]),
                        "train/avg_param_mse": float(train_dict["avg_param_mse"]),
                        "train/avg_rel_l2": float(train_dict["avg_rel_l2"]),
                        "val/avg_field_mse": val_field_mse,
                        "val/avg_param_mse": val_param_mse,
                        "val/avg_rel_l2": val_rel_l2,
                        "test/avg_field_mse": test_field_mse,
                        "test/avg_param_mse": test_param_mse,
                    ...
                    if best_val_results is not None and best_test_results is not None:
                        ...
                        eval_log["val/best_field_mse"] = float(best_val_results["avg_field_mse"])
                        eval_log["val/best_param_mse"] = float(best_val_results["avg_param_mse"])

                        eval_log["test/best_at_best_val_rel_l2"] = float(best_test_results["avg_rel_l2"])
                        eval_log["test/best_at_best_val_field_mse"] = float(best_test_results["avg_field_mse"])
                        eval_log["test/best_at_best_val_param_mse"] = float(best_test_results["avg_param_mse"])
